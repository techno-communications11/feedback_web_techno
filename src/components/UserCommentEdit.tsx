...
const MarketManageredit = ( {comment_id}:CommentProps) => {
  ...
  const handleSave = async () => {
    try {
       if(!comment_id){
        throw new Error("Comment ID is required to save the comment");
       }
       console.log("Saving comment:", commentText, comment_id);
      // Example: send comment to server (replace with your endpoint)
      const res = await fetch("/api/usercommentedit", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ comment_text: commentText, comment_id  }),
      });
